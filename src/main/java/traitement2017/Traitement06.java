...
public class Traitement06 implements LigneListener{
	...
	public void ligneLue(String ligne) {
		...
		/* Création d'un Set de String où l'on va stocker les listes d'Integer successives
		 * sous forme de String ce qui permet de voir à quel moment on essaiera d'insérer une liste
		 * qu'on a déjà insérer, on aura un false en retour du add */
		...
		do {
			...
			// distribution du max 1 par 1 dans les éléments suivants de liste
			// jusqu'à ce que le max vaille 0 
			while(max>0){
				// tant qu'on n'est pas sur le dernier élémennt de la liste
				if(positionCourante<liste.size()-1){
					// on ajoute 1 à l'élément suivant
					liste.set(positionCourante+1, liste.get(positionCourante+1)+1); 
				...
				// on enlève 1 à l'élément qui a été identifié comme max au début du tour de boucle
				liste.set(position, liste.get(position)-1);
				max--; // on on enlève 1 au max a distribuer
			}
			// on continue à réorganiser la liste jusqu'à ce qu'on arrive sur une liste qu'on a déjà eu,
			// auquel cas le add du Set va retourner false
		} while(setDeListInteger.add(conversionListEnString(liste)));

		// On veut maintenant savoir combien il y a de tours de boucles
		// entre les 2 listes identiques (tailleCycle)
		// on récupère la dernière qui est celle qu'on n'a pas pu insérer dans le Set
		// et on recherche sa position dans le Set
		...
		// calcul de la taille du cycle en faisant la différence entre
		// le nombre de tours de boucle total moins la position de la liste identique dans le Set  
